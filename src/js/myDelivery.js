$(function(){
	...
	if(partname == "//myDelivery.html" || partname == "/myDelivery.html"){
		pageInf.carImgContactTel = logininf.mobilePhone;
		orderListFun(pageInf,"承运商");
	}
	if(partname == "//myShipments.html" || partname == "/myShipments.html"){
	...
	function getSearchInf(isGetTime,startmouthnum,endmouthnum){
		...
		if(isGetTime == 1){
            ...
            pageInf.endCompleteTime = getCurrentTime(endmouthnum*30);
            pageInf.startCompleteTime = getCurrentTime(startmouthnum*30);
		}
	...
	function orderListFun(pageInf,currentRole) {
		$.ajax({
			url: tmsUrl + '/salesman/query/SalesmanHeadOrderTaskInfoPage',
			type: "post",
			contentType: 'application/json',
			beforeSend:function(){
    ...
    function getLogisticsInfo(logisticslist){
        ...
        for(var i = 0; i < logisticslist.length;i++){
            ...
            logisticsLi += '<li>'+
                ...
                '<p class="inR">备注：'+logisticslist[i].remark+'</p>'+
                '</li>'
        }
