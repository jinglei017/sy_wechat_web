$(function(){
	...
	$.ajax({
	    ...
	    success: function(data) {
	    	...
	    	if(data.result.actInfoModel.actCurrent.latLng == "" || data.result.actInfoModel.actCurrent.latLng == null ||data.result.actInfoModel.actCurrent.latLng == "null"){
                /*if(data.result.sfrLocationCode != null){
                    getCurrentLocationCode(data.result.sfrLocationCode);
                }*/
	...
	function getLogisticsInfo(logisticslist){
		var logisticsLi = "";
		//var statusArr = {"RECV":"接收","COFM":"确认","SENT":"已发出","RJCT":"拒收","SPLIT":"拆单","ACPT":"验收","DCHT": "卸车","LONT":"装车","SEND":"发送","DONE":"完成","INIT":"初始化","DIST":"接单","EXCP":"异常"};
	...
	function getOrderInfo(orderinfo){
		...
		if(orderinfo.exceptionStatus == "1" || orderinfo.exceptionStatus == "2"){
			...
			var orderdetailinfo = '<ul>'+
				...
				'<p class="inf" style="color: red;">'+orderinfo.exceptionStatus+'</p>'+
				'</li>'+
                paymentItem+
				'<li>'+
